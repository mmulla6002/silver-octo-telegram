...
class Game():
    def __init__(self, screen, clock, game_length, difficulty):
        global screen
        global clock
        ...
        class AI_Bike(Bike):
            ...
            def CalculateTarget(self,player):
                if (abs(player.x - self.x) < 80) and (abs(player.y - self.y) < 80):
                    self.CloseEncounter()
                    continue
                path = A_STAR.main(self.x//40, self.y//40, player.x//40, player.y//40)
                if path == None:
                    self.Retreat()
                    continue
                ...
                if player.y > 100:
                    path = A_STAR.main(self.x//40, self.y//40, self.targetx//40, self.targety//40)
                ...
                try:
                    if trail.parent.rect.colliderect(trail.rect) == False:
                        ...
                        if ((trail.x - 15) % 40) == 0 and ((trail.y - 15) % 40) == 0:
                            A_STAR.maze[trail.y // 40][trail.x // 40] = 1
